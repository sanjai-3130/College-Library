...
const StaffCardVerify: React.FC = () => {
  // Extract URL Parameters
  const [params, setParams] = useState({
    id: 'STF-2024-8801',
    name: 'Dr. Elizabeth Vance',
    dept: 'Library Operations',
    year: 'Faculty Staff',
    semester: 'Staff Coordinator',
    section: 'A',
    blood: 'A+',
    phone: '9876543210',
    email: 'vance.elizabeth@college.edu',
    avatar: '',
    role: 'staff'
  });
  ...
  useEffect(() => {
    ...
    const id = urlParams.get('id') || 'STF-2024-8801';
    const name = urlParams.get('name') || 'Dr. Elizabeth Vance';
    const dept = urlParams.get('dept') || 'Library Operations';
    const year = urlParams.get('year') || 'Faculty Staff';
    const semester = urlParams.get('semester') || 'Staff Coordinator';
    const section = urlParams.get('section') || 'A';
    const blood = urlParams.get('blood') || 'A+';
    const phone = urlParams.get('phone') || '9876543210';
    const email = urlParams.get('email') || 'vance.elizabeth@college.edu';
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Normalize to max 12 degrees tilt
  ...
  return (
    <div className="min-h-screen w-full relative flex items-center justify-center p-4 md:p-8 bg-slate-50 overflow-x-hidden selection:bg-amber-500 selection:text-white font-sans">
      {/* Dynamic Background Blurs */}
      <div className="absolute top-[-10%] left-[-10%] w-[400px] md:w-[600px] h-[400px] md:h-[600px] rounded-full bg-amber-500/10 blur-[100px] md:blur-[150px] pointer-events-none animate-pulse duration-5000" />
      ...
      <div className="perspective-1200 w-full flex justify-center items-center py-10">
        ...
        >
          {/* Top header gold-amber ribbon for faculty/staff */}
          <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-amber-500 to-indigo-500" />

          {/* Institutional Branding Header */}
          <div className="flex items-center justify-center gap-3 mb-6 w-full">
            <div className="h-10 w-10 bg-gradient-to-br from-amber-500 to-indigo-600 rounded-xl flex items-center justify-center text-white shadow-md shadow-amber-100">
          ...
          <div className="relative group mb-4">
            <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-amber-400 to-indigo-400 blur-sm opacity-50 group-hover:opacity-100 transition duration-1000 group-hover:duration-200" />
          ...
          {/* Secure Verified Badge (Amber Gold) */}
          <div className="px-3.5 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1.5 shadow-sm uppercase tracking-wider mb-6 bg-amber-500 text-white shadow-amber-100">
            <ShieldCheck className="h-3.5 w-3.5 stroke-[2.5]" />
            <span>Verified Faculty / Staff</span>
          </div>
          ...
          <div className="w-full space-y-2.5 bg-slate-50/50 rounded-2xl border border-slate-200/40 p-4 mb-6">
            ...
            <div className="flex justify-between items-center py-1.5 border-b border-slate-200/40 text-xs">
              ...
              <span className="font-bold text-slate-800">{params.semester || 'Faculty Staff'}</span>
            </div>
            ...
            <div className="flex justify-between items-center py-1.5 border-b border-slate-200/40 text-xs">
              ...
              <span className="font-bold text-slate-800">Main Campus, Block C</span>
            </div>
          ...
          <div className="w-full flex flex-col items-center pt-4 border-t border-slate-150">
            ...
            <div className="flex items-center gap-1.5 text-slate-400 font-bold text-[10px] uppercase tracking-widest font-mono">
              <Sparkles className="h-3.5 w-3.5 text-amber-500 fill-amber-500/20 animate-spin duration-3000" />
              <span>Digital Faculty ID</span>
            </div>
          </div>
        </div>
      </div>
...
if (rootElement) {
  createRoot(rootElement).render(<StaffCardVerify />);
}
